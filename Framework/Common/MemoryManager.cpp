...
void GE::MemoryManager::Finalize()
{
    delete[] m_pAllocators;
    delete[] m_pBlockSizeLookup;
}

void GE::MemoryManager::Tick()
{
    
}

Allocator* GE::MemoryManager::LookUpAllocator(size_t size)
{
...
void* GE::MemoryManager::Allocate(size_t size)
{
...
void* GE::MemoryManager::Allocate(size_t size, size_t alignment)
{
    uint8_t* p;
    size += alignment;
    Allocator* pAlloc = LookUpAllocator(size);
...
void GE::MemoryManager::Free(void* p, size_t size)
{
    Allocator* pAlloc = LookUpAllocator(size);
    if(pAlloc){
        pAlloc->Free(p);
