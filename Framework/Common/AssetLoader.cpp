...
int GE::AssetLoader::Initialize()
{
    return 0;
}

void GE::AssetLoader::Finalize()
{
    m_strSearchPath.clear();
}

void GE::AssetLoader::Tick()
{
    
}

bool GE::AssetLoader::AddSearchPath(const char* path)
{
...
bool GE::AssetLoader::RemoveSearchPath(const char* path)
{
    std::vector<std::string>::iterator src = m_strSearchPath.begin();
...
bool GE::AssetLoader::FileExists(const char* filePath)
{
...
GE::AssetLoader::AssetFilePtr GE::AssetLoader::OpenFile(const char* name, AssetOpenMode mode)
{
    FILE *fp = nullptr;
...
GE::Buffer GE::AssetLoader::SyncOpenAndReadText(const char* filePath)
{
...
void GE::AssetLoader::CloseFile(AssetFilePtr& fp)
{
    fclose((FILE*)fp);
    fp = nullptr;
}

size_t GE::AssetLoader::GetSize(const AssetFilePtr& fp)
{
...
size_t GE::AssetLoader::SyncRead(const AssetFilePtr& fp, Buffer& buf)
{
    size_t sz;
...
int32_t GE::AssetLoader::Seek(AssetFilePtr fp, long offset, AssetSeekBase where)
{
