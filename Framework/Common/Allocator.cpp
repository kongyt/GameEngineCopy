...
GE::Allocator::Allocator(size_t data_size, size_t page_size, size_t alignment)
    : m_pPageList(nullptr), m_pFreeList(nullptr)
{
...
void GE::Allocator::Reset(size_t data_size, size_t page_size, size_t alignment)
{
    FreeAll();
...
#if defined(_DEBUG)
void GE::Allocator::FillFreePage(PageHeader* pPage)
{
    // page header
    pPage->pNext = nullptr;
...
void GE::Allocator::FillFreeBlock(BlockHeader* pBlock)
{
    // block header + data
...
void GE::Allocator::FillAllocatedBlock(BlockHeader* pBlock)
{
    // block header + data
...
GE::BlockHeader* GE::Allocator::NextBlock(BlockHeader* pBlock)
{
